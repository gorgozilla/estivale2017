...
function acymailing_frontendLink($link, $newsletter = true, $popup = false){
	...
	if($config->get('use_sef', 0) && strpos($link, '&ctrl=url') === false){
		...
		$json = json_decode(acymailing_fileGetContent(JURI::root().'index.php?option=com_acymailing&ctrl=url&task=sef&urls[0]='.base64_encode($link)), true);
...
function acymailing_fileGetContent($url, $timeout = 10){
	...
	if(class_exists('JHttpFactory') && method_exists('JHttpFactory', 'getHttp')) {
		...
		try {
			$response = $http->get($url, null, $timeout);
		} catch (RuntimeException $e) {
			$response = null;
		}

		if ($response !== null && $response->code === 200) return $response->body;
	}
